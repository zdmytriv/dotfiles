...
def list_envs(aws_profile):
    ...
    for ebt_env in ebt_envs:
        ebt_instances = \
            ebt_client.describe_environment_resources(EnvironmentId=ebt_env['EnvironmentId'])['EnvironmentResources'][
                'Instances']
        ...
        env = [", ".join(ip_addresses), env_name, cname, version_label, "%s|%s|%s" % (health, health_status, status)]
        result.append(env)
